...
static struct config** findNode(const config_t* one, const char* key) {
    if (!one) { return NULL; }
    if (!one->confs) { return NULL; }
    if (!key) { return NULL; }

    if (!strcmp(one->confs->name, key)) { return (struct config**)&one->confs; }
...
struct config* newConfig(const char* key, const char* val) {
    ...
    if (keyLen <= 1 || valLen <= 1) { return NULL; }
...
void confDestory(config_t* one) {
    ...
    while (one->confs) { freeConfigOne(&one->confs); }
    free(one);
    one = NULL;
}
...
int confSet(config_t* one, const char* key, const char* val) {
    ...
    if (one->confs) { node->next = one->confs; }
...
config_t* confRead(const char* file_name) {
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        // 新的配置项
        if (key && val) { confSet(one, key, val); }
    }
